...
// Add import for update hook and get announcement by ID
...
export default function EditAnnouncementScreen() {
  const { menuAnnouncementId } = useLocalSearchParams();
  const announcementId = menuAnnouncementId as string;
  
  const { mutate: updateAnnouncement, isPending } = useUpdateAnnouncement();
  const { data: events = [] } = useGetAllEvents();
  const { data: existingAnnouncement, isLoading: isLoadingAnnouncement, isError } = useGetAnnouncementById(announcementId);
  ...
  // Load existing announcement data when it's available
  useEffect(() => {
    if (existingAnnouncement) {
      setFormData({
        title: existingAnnouncement.title || "",
        message: existingAnnouncement.message || "",
        event: existingAnnouncement.event || null,
        expiryDate: existingAnnouncement.expiryDate || "",
        isUrgent: existingAnnouncement.isUrgent || false,
      });
      setAttachments(existingAnnouncement.attachments || []);
    }
  ...
  // Change submit handler to use update instead of create
  const handleSubmit = () => {
    ...
    updateAnnouncement(
      { id: announcementId, data: announcementData },
      {
        onSuccess: () => {
  ...
  if (isError) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.content}>
          <Text style={styles.sectionTitle}>Error loading announcement.</Text>
        </View>
      </SafeAreaView>
    );
  }
